...
def stubborn_asker(low, high):
    ...
    """
    number = None
    while number is None or not (low <= number <= high):
...
def super_asker(low, high):
    ...
    """
    message = f"Please enter a number between {low} and {high}: "
    while True:
        response = input(message)
        try:
            number = float(response)
            if low <= number <= high:
                ...
                print("The number is outside the specified range. Try again.")
        except ValueError:

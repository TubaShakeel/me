...
def wordy_pyramid():
    ...
    for length in range(min_length, max_length + 1, step):
        ...
        if response.status_code == 200:
            ...
            pyramid.append(None)  # Add a placeholder if request fails
...
def diarist():
    ...
    file_path = "Trispokedovetiles(laser).gcode"
    with open(file_path, "r") as gcode_file:
        gcode_content = gcode_file.read()

    # Count the occurrences of the command "M10 P1"
    command_to_count = "M10 P1"
    command_count = gcode_content.count(command_to_count)

    # Write the count to 'lasers.pew' file
    output_file_path = "Set4/lasers.pew"
    with open(output_file_path, "w") as output_file:
        output_file.write(str(command_count))
